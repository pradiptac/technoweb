 ...
 * everything is back at zero; (4) hovering the Store link does *not* stop
 * it, opening the shop does — `data-quiet` and the animation is `none`;
 * (5) a reload keeps it quiet
...
// The site popup opens after its own delay over the header; close any dialog
// the moment it opens rather than guessing when.
...
ok((await badge.getAttribute("data-quiet")) === null, "hovering Store does not stop it");
await page.locator("header a[href='/store']").first().click();
await page.waitForURL((u) => u.pathname.startsWith("/store"), { timeout: 60000 });
await page.waitForSelector("header .cart-burst[data-quiet]", { timeout: 10000 }).catch(() => {});
ok((await page.locator("header .cart-burst").first().getAttribute("data-quiet")) !== null, "opening the shop stamps data-quiet");
ok((await page.locator("header .cart-burst").first().evaluate((el) => getComputedStyle(el).animationName)) === "none", "and the animation is none");
// The shop's page is the heaviest on the site under a dev server; wait for the
// document rather than every image.
