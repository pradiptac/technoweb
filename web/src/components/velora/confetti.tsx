...
/**
 * Velora `confetti` — https://velora.colorlib.com/r/confetti.json, installed
 * 2026-09-14 (Velora by Colorlib). As published, with `React.*` types
 * imported by name. `brandConfettiColors()` is ours: the published default
 * palette is violet, pink and cyan, and a canvas cannot read a CSS variable,
 * so the caller resolves this theme's tokens at fire time.
 */
...
/** This theme's brand, secondary and accent steps, read from the live tokens. */
export function brandConfettiColors(): string[] {
  const cs = getComputedStyle(document.documentElement);
  const read = (k: string) => cs.getPropertyValue(k).trim();
  return ["--color-brand-400", "--color-brand-500", "--color-accent-400", "--color-accent-500", "--color-secondary-400", "--color-brand-300"]
    .map(read)
    .filter(Boolean);
}

/**
 * Fire a confetti burst â€” no canvas library, ~2s, cleans up after itself.
