...
/**
 * The hand-off mark on the assistant's header.
 *
 * A `brand` mark — filled, `currentColor` — because it is a *logo* and the set
 * above is where logos live, not an outline icon that happens to be a
 * telephone. It takes its colour from the control it sits in rather than
 * WhatsApp's own green, for the reason every literal in this codebase is a
 * token: that green does not invert with the scheme, and the button around it
 * does.
 *
 * Deliberately **not** in `iconMap`. That map is identity icons, which take a
