...
import { resetStaffPassword } from "@/lib/admin-auth";
import type { ResetState } from "@/components/forms/reset-password-form";

export async function resetStaffPasswordAction(_prev: ResetState, formData: FormData): Promise<ResetState> {
  const payload = {
  ...
  try {
    await resetStaffPassword(payload);
  } catch (error) {
