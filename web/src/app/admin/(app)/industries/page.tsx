  ...
  return (
    <>
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <h2 className="display-3">Industries</h2>
      ...
      <form className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-line-strong bg-white p-4" action="/admin/industries">
        <div className="min-w-[150px]">
          <label htmlFor="q" className="mb-1.5 block text-[12px] font-semibold text-muted">Search</label>
          <Input id="q" name="q" defaultValue={params.q} placeholder="Name or summary…" className="min-w-[260px]" />
        </div>
          ...
          <table className="w-full min-w-[680px] text-left text-[13.5px]">
            <thead>
              <tr className="border-b border-line-strong text-[12px] font-semibold uppercase tracking-[.04em] text-muted">
                ...
                <th scope="col" className="px-4 py-3">Case studies</th>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <Link href={`/admin/industries/${i.id}`} className="block hover:underline">
                          <p className="max-w-[42ch] text-[14px] text-ink">{i.name}</p>
                        </Link>
                        <p className="mt-0.5 font-mono text-[12px] text-muted">/industries/{i.slug}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-muted">{i.case_study_count ?? 0}</td>
                  <td className="px-4 py-3.5 font-mono text-[12.5px] text-muted">{i.sort_order ?? 0}</td>
                </tr>
              ))}
            </tbody>
          </table>
