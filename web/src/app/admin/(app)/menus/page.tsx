    ...
    <>
      ...
      */}
      <section className="mb-6">
        <h2 className="mb-2 text-13 font-semibold">Where menus appear</h2>
            ...
            return (
              <li key={location.value} className="rounded-lg border border-line-strong bg-card p-3.5">
                <div className="mb-1 flex items-center gap-2">
                  <h3 className="text-13 font-semibold">{location.label}</h3>
                  {menu
                    ? <Badge tone="resolved">Assigned</Badge>
                    : <Badge tone="closed">Built-in</Badge>}
                </div>
                <p className="measure mb-2 text-12-5 text-muted">{location.hint}</p>
                {menu
                  ? <Link href={`/admin/menus/${menu.id}`} className="text-12-5 font-semibold text-brand-ink hover:underline">
                      Edit “{menu.name}” →
                    </Link>
                  : <p className="text-12-5 text-faint">
      ...
      <h2 className="mb-2 text-13 font-semibold">All menus</h2>
        ...
        <ul className="grid gap-2">
          {menus.map((menu) => (
            <li key={menu.id}>
              ...
              >
                <span className="min-w-0 flex-1 basis-full truncate text-13 font-medium sm:basis-auto">{menu.name}</span>
                {menu.location_label
                  ? <Badge tone="resolved">{menu.location_label}</Badge>
                  : <Badge tone="closed">Not assigned</Badge>}
                <span className="shrink-0 text-12-5 text-faint tabular-nums">
